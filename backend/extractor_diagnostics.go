...
func GetExtractorDiagnosticsSnapshot() ExtractorDiagnosticsSnapshot {
	privateReason, privatePinned := timelineAutoBypassReason(TimelineRequest{TimelineType: "likes"})
	...
	publicGate, privateGate, publicFamilyGates := resolveExtractorValidationGates(runbookConfig, recentValidationReports)
	liveFamilyGates, promotionFamilyGates := resolveExtractorLiveValidationGates(runbookConfig, recentLiveReports)
...
func buildExtractorSupportMatrixSummary() ExtractorSupportMatrixSummary {
	return ExtractorSupportMatrixSummary{
		...
		PrivateAutoPinnedTimeline:   []string{"likes", "bookmarks"},
